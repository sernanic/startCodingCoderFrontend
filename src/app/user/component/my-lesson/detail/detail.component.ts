...
export class LessonDetailComponent implements OnInit {
  ...
  chooseSlot(time: any) {
    if (window.confirm('Are you sure to reschedule to this slot?')) {
      const data = {
      ...
      this.appointmentService
        ...
        .then(resp => {
          if (!resp.data.checkOverlap) {
            if (window.confirm('This slot is overlap with your booked slot. Still reschedule to it?')) {
              this.appointmentService
                .reSchedule(this.appointment._id, data)
                .then(resp => {
                  this.toasty.success('Reschedule successfully');
                  this.appointment.startTime = time.start;
                  this.appointment.toTime = time.end;
