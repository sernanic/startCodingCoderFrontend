...
export class MyCategoriesComponent implements OnInit {
  ...
  queryCategories(params) {
    this.categoryService.getCategories({ ...params, take: 1000 }).then(resp => {
  ...
  querySubjects() {
    this.subjectService.getSubjects({ categoryIds: this.selectedCategory._id, take: 1000 }).then(resp => {
      if (resp.data && resp.data.items && resp.data.items.length > 0) {
  ...
  queryTopic(params) {
    this.topicService.getTopics({ ...params, take: 1000 }).then(resp => {
      if (resp.data && resp.data.items && resp.data.items.length > 0) {
