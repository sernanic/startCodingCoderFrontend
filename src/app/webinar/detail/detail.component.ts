...
export class DetailWebinarComponent implements OnInit {
  ...
  findSlots() {
    this.calendarService
      .search({ webinarId: this.webinarId, take: 100 })
      .then(async resp => {
  ...
  enrollWebinar(webinar: any, type: string) {
    if (!this.auth.isLoggedin()) {
      return this.toasty.error('Please login to enroll the webinar!');
    }
    if (this.webinar.numberParticipants >= this.webinar.maximumStrength) {
      return this.toasty.error('No slot available!');
    }
    this.webinarService
  ...
  favorite() {
    if (!this.isLoggedin) this.toasty.error('Please loggin to use this feature!');
    else {
      ...
      this.webinarFavoriteService
        ...
        .then(res => {
          ...
          this.toasty.success('Added to your favorite webinar list successfully!');
        })
    ...
    if (!this.isLoggedin) this.toasty.error('Please loggin to use this feature!');
      ...
      this.webinarFavoriteService
        ...
        .then(res => {
          ...
          this.toasty.success('Deleted from your favorite webinar list successfully!');
        })
        .catch(() => this.toasty.error('Something went wrong, please try again!'));
    }
  }
