...
export class SubjectComponent implements OnInit {
  ...
  queryTopic() {
    this.topicService.search({ subjectIds: this.searchFields.subjectIds, take: 1000 }).then(resp => {
  ...
  querySubjects() {
    this.subjectService.search({ categoryIds: this.searchFields.categoryIds, take: 1000 }).then(resp => {
      if (resp.data && resp.data.items && resp.data.items.length > 0) {
