...
export class ApplyCouponComponent implements OnInit {
  ...
  constructor(private couponService: CouponService, private toasty: ToastrService, private authService: AuthService) {}
  ...
  applyCoupon() {
    ...
    if (!this.options.topicId && this.options.targetType === 'subject') {
      return this.toasty.error('Please choose category, subject and topic first!');
    }
